...
JOG_FEED_XY = 1600  # X/Y plus rapides pour jog continu et calibration
...
CONTINUOUS_JOG_DISTANCE_XY = 220.0
...
def send_jog(axis, distance, feed):
    ...
    # Inversion logicielle de Z seulement : le dashboard reste logique.
...
def send_continuous_jog(axis, direction):
    """Jog continu : la machine bouge tant que le bouton reste appuyé.
    Le dashboard lance /jog_start au pointerdown et /jog_stop au relâchement.
    GRBL reçoit une seule commande $J mono-axe avec une distance longue, puis 0x85 annule le jog.
    """
    axis = str(axis).upper().strip()
    ...
    long_dist = CONTINUOUS_JOG_DISTANCE_Z if axis == "Z" else CONTINUOUS_JOG_DISTANCE_XY
    ...
    result = send_raw(cmd, timeout=1.5)
    return {
        "line": cmd,
        "response": result.get("response", "unknown"),
        "raw": [result],
        "mode": "continuous_jog_start_mono_axis"
    }



def map_z_for_grbl(line):
    """Inverse Z dans les commandes de mouvement G0/G1 envoyées à GRBL.
...
def send_gcode_lines(lines):
    ...
    Amélioration importante : l'ancienne version envoyait les lignes rapidement et
    s'arrêtait dès qu'un timeout apparaissait. Ici, chaque mouvement G0/G1 et chaque
    attente G4 sont suivis par une attente réelle de l'état Idle. Cela évite que le bec
    saute des points ou que la séquence s'interrompe avant la fin.
    """
    stop_event.clear()
    results = []
    connect_grbl()
    ...
    for original in lines:
        ...
        if original_str.startswith("; Point"):
            ...
            # On ne l'envoie pas à GRBL, mais on le garde dans les résultats.
        ...
        if resp.startswith("ok"):
            ...
            # Pour les commandes critiques, attendre la fin réelle du mouvement.
            if is_motion_or_wait_command(line):
                ...
                if not idle_ok:
                    result["response"] = "idle_timeout"
                    results.append(result)
                    break

            # Compter le point après la remontée Z sécurité qui suit le contact soudure.
            if current_point_comment and line.upper().startswith(("G1", "G01")) and "Z" in line.upper():
                try:
                    zval = extract_axis_values(line).get('z', None)
                    if zval is not None and zval >= current_pos.get('z', 0.0):
                        pass
                except Exception:
                    pass
        results.append(result)
...
def set_machine_origin():
    try:
        results = send_gcode_lines(["G21", "G90", "G92 X0 Y0 Z0"])
        current_pos.update({"x": 0.0, "y": 0.0, "z": 0.0})
        return jsonify({"status": "ok", "message": "Position actuelle enregistree comme origine machine X0 Y0 Z0.", "results": results})
    except Exception as e:
        return jsonify({"status": "error", "message": str(e)}), 500
...
def homing_return_manual_origin():
    try:
        ...
        results = send_gcode_lines(lines)
        return jsonify({"status": "ok", "message": "Retour origine machine manuelle termine.", "results": results})
    except Exception as e:
        return jsonify({"status": "error", "message": str(e)}), 500
...
def read_grbl_steps_internal():
    """Lit $$ et retourne settings + steps. Utilisé par l'interface et par la calibration."""
    result = send_raw("$$", timeout=5.0)
    settings = {}
    steps = {}
